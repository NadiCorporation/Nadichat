package com.nadichat.android.services
...
class MessageRetentionService private constructor(private val context: Context) {
    ...
    suspend fun saveMessage(message: NadichatMessage, forChannel: String) = withContext(Dispatchers.IO) {
    ...
    suspend fun loadMessagesForChannel(channel: String): List<NadichatMessage> = withContext(Dispatchers.IO) {
    ...
    private fun loadMessagesFromFile(file: File): List<NadichatMessage> {
        if (!file.exists()) {
            return emptyList()
        }
        ...
        return try {
            FileInputStream(file).use { fis ->
                ObjectInputStream(fis).use { ois ->
                    ...
                    ois.readObject() as List<NadichatMessage>
                }
            }
        } catch (e: Exception) {
    ...
    private fun saveMessagesToFile(file: File, messages: List<NadichatMessage>) {
