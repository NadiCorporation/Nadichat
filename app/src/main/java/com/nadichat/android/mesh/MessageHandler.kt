...
class MessageHandler(private val myPeerID: String) {
    ...
    private suspend fun handleBroadcastMessage(routed: RoutedPacket) {
        ...
        try {
            // Parse message
            val message = NadichatMessage.fromBinaryPayload(packet.payload)
            if (message != null) {
     ...
     */
    private suspend fun handlePrivateMessage(packet: NadichatPacket, peerID: String) {
        try {
            ...
            // Parse message
            val message = NadichatMessage.fromBinaryPayload(unpaddedData)
            if (message != null) {
                // Check for cover traffic (dummy messages)
                if (message.content.startsWith("☂DUMMY☂")) {
     ...
     */
    private fun sendDeliveryAck(message: NadichatMessage, senderPeerID: String) {
        handlerScope.launch {
...
interface MessageHandlerDelegate {
    ...
    // Packet operations
    fun sendPacket(packet: NadichatPacket)
    fun relayPacket(routed: RoutedPacket)
    fun getBroadcastRecipient(): ByteArray
    
    // Cryptographic operations
    fun verifySignature(packet: NadichatPacket, peerID: String): Boolean
    ...
    // Callbacks
    fun onMessageReceived(message: NadichatMessage)
