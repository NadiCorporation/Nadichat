...
class BluetoothStatusManager(
    ...
    private fun setupBluetoothEnableLauncher() {
        bluetoothEnableLauncher = activity.registerForActivityResult(
            ...
            if (isEnabled) {
                ...
                onBluetoothDisabled("Bluetooth is required for Nadichat to discover and connect to nearby users. Please enable Bluetooth to continue.")
            }
        }
    }
    ...
    fun handleBluetoothStatus(status: BluetoothStatus) {
        when (status) {
            ...
            BluetoothStatus.NOT_SUPPORTED -> {
                ...
                onBluetoothDisabled("This device doesn't support Bluetooth, which is required for Nadichat to function.")
            }
        }
    }
    ...
    fun getStatusMessage(status: BluetoothStatus): String {
        return when (status) {
            ...
            BluetoothStatus.DISABLED -> "Bluetooth is disabled. Please enable Bluetooth to use Nadichat."
