package com.nadichat.android.onboarding
...
/**
 * Centralized permission management for Nadichat app
...
class PermissionManager(private val context: Context) {
    ...
    companion object {
        ...
        private const val PREFS_NAME = "nadichat_permissions"
    ...
    fun getCategorizedPermissions(): List<PermissionCategory> {
        ...
        categories.add(
            PermissionCategory(
                ...
                description = "Required to discover Nadichat users via Bluetooth",
                permissions = bluetoothPermissions,
                isGranted = bluetoothPermissions.all { isPermissionGranted(it) },
                systemDescription = "Allow Nadichat to connect to nearby devices"
            )
        )
        ...
        categories.add(
            PermissionCategory(
                ...
                description = "Required by Android to discover nearby Nadichat users via Bluetooth",
                permissions = locationPermissions,
                isGranted = locationPermissions.all { isPermissionGranted(it) },
                systemDescription = "Nadichat needs this to scan for nearby devices"
            )
        )
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.TIRAMISU) {
            categories.add(
                PermissionCategory(
                    ...
                    systemDescription = "Allow Nadichat to send you notifications"
                )
            )
        }
